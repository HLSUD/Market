...
//操作员账户
const char *UserID = "00032967";	   //同客户号保持一致即可
...
private:
	...
	void OnRspUserLogin(CTORATstpRspUserLoginField *pRspUserLoginField, CTORATstpRspInfoField *pRspInfo, int nRequestID)
	{
		...
		if (pRspInfo->ErrorID == 0)
		{
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqUserPasswordUpdate fail, ret[%d]\n", ret);
			}
			return;
#endif
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqQrySecurity fail, ret[%d]\n", ret);
			}
			return;
#endif
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqQryInvestor fail, ret[%d]\n", ret);
			}
#endif
			...
			if (ret_sa != 0)
			{
				fprintf(fp, "ReqQryShareholderAccount fail, ret[%d]\n", ret_sa);
			}
			...
			if (ret_ta != 0)
			{
				fprintf(fp, "ReqQryTradingAccount fail, ret[%d]\n", ret_ta);
			}
#endif

#if 1
			// 查询报单
			CTORATstpQryOrderField c_field;
			...
			int c_ret = m_api->ReqQryOrder(&c_field, m_req_id++);
			if (c_ret != 0)
			{
				fprintf(fp, "ReqQryOrder fail, ret[%d]\n", c_ret);
			}
#endif
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqQryPosition fail, ret[%d]\n", ret);
			}
#endif
			...
			if (recall_ret != 0)
			{
				fprintf(fp, "ReqOrderAction fail, ret[%d]\n", recall_ret);
			}
#endif
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqInquiryJZFund fail, ret[%d]\n", ret);
			}
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqTransferFund fail, ret[%d]\n", ret);
			}
#endif
			...
			if (ret != 0)
			{
				fprintf(fp, "ReqUserLogout fail, ret[%d]\n", ret);
			}
#endif      
            fclose(fp);
			return;
	...
	virtual void OnRspOrderInsert(CTORATstpInputOrderField *pInputOrderField, CTORATstpRspInfoField *pRspInfo, int nRequestID)
	{
		...
		if (pRspInfo->ErrorID == 0)
		{
			fprintf(fp, "OnRspOrderInsert: OK! [%d] [%d] [%s]\n", nRequestID, pInputOrderField->OrderRef, pInputOrderField->OrderSysID);
		}
		else
		{
			fprintf(fp, "OnRspOrderInsert: Error! [%d] [%d] [%s]\n", nRequestID, pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		}
        fclose(fp);
	}
	...
	virtual void OnRspQryOrder(CTORATstpOrderField *pOrder, CTORATstpRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		if (bIsLast)
		{
			fprintf(fp,"Query order end[%d] ErrorID[%d] ErrorMsg[%s]\n", nRequestID, pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		}
        fclose(fp);
	}
